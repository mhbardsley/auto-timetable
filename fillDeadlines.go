...
func hasFilled(timetable []timetableElement, deadlines []deadline, pow int) bool {
	...
	for i, slot := range timetable {
		if slot.event == nil && len(deadlines) > 0 {
			weights := getWeights(deadlinesCopy, i)
...
func reduceDeadlines(deadlines []deadline, index int) {
	zeroFlag := false
	for i, deadline := range deadlines {
	...
	if zeroFlag {
		_ = append(deadlines[:index], deadlines[index+1:]...)
	}
}
